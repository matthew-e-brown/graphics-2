#![allow(unused)] // [TODO] remove this once "new" core is complete

/// A convenience macro for converting Rust-native expressions into [GL-types][gl::types] with `try_from`.
///
...
/// let norm = convert!(false, GLboolean, "'normalized' parameter");
/// ```
///
/// The third parameter is normally given simply as a name for the type. This will cause `.expect` to print out,
/// ""[name] should fit into a '[type]'". For example:
///
/// ```should_panic
...
/// ```
/// # let buffer = [1];
/// let size = convert!(buffer.len(), GLsizei, msg: "your buffer is too large!!");
/// ```
macro_rules! convert {
