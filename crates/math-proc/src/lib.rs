...
//! # Functionality
//!
//! /// Because not all possible inner-types of a vector implement all the required traits (e.g., a vector of `bool`s
//! /// has no dot product, since `bool`s cannot be multiplied), additional functionality is implemented by other macros
//! /// from [this crate](self).
//!
...
//!     indexed row-first).
//! - Free conversions between vectors and arrays of their held type, and between matrices and two-dimensional arrays of
//!   their held type. This is done using a transmute. Because all vectors and matrices are simply wrappers around
//!   arrays of (usually) scalars, **all vectors and matrices are `repr(transparent)`**, which guarantees that a
//!   transmute is safe.
...
//! - Separated by a semicolon, the type that the matrix or vector should contain (any scalar type should work).
//! - Separated by comma(s), the size of the matrix or vector as a [`usize`] literal.
//!
//! Because attributes are passed through with the macro invocation, derive macros like [`Clone`] and [`Copy`] should be
//! included here; they are not derived automatically.
//!
//! Documentation is generated for the struct itself automatically.
//!
//!
//! ### Examples
//!
